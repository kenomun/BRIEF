  ...
  try {
    ...
    let test = await prisma.test.create({
      data: {
        ...
        subjectId: subjectId,
        Questions: {
          create: await Promise.all(questions.map(async (question) => {
...
const getAllTests = async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Error al obtener los tests", details: error.message });
  }
};
...
const getTestById = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: "Error al obtener el test", details: error.message });
  }
};
...
const getTestsBySubjectId = async (req, res) => {
  try {
    ...
    if (!id) {
      return res.status(400).json({ message: 'El ID de la asignatura es obligatorio.' });
    }
    ...
    if (tests.length === 0) {
      return res.status(404).json({ message: 'No se encontraron tests para esta asignatura.' });
    }
...
const deleteTest = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({ message: 'Test y todas las relaciones eliminadas correctamente.' });
